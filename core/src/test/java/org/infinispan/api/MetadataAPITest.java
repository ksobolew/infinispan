...
public class MetadataAPITest extends SingleCacheManagerTest {
   ...
   public void testPutWithLifespan() {
      ...
      advCache.put(key, "v1", new EmbeddedMetadata.Builder().lifespan(lifespan).build());
      CacheEntry cacheEntry = advCache.getCacheEntry(key);
   ...
   public void testConditionalReplaceWithLifespan() {
      ...
      advCache.replace(key, "v1", "v2", new EmbeddedMetadata.Builder().lifespan(newLifespan).build());
      CacheEntry cacheEntry = advCache.getCacheEntry(key);
   ...
   public void testPutIfAbsentWithLifespan() {
      final Integer key = 3;
      long lifespan = 1_000_000;
      assertEquals(null, advCache.putIfAbsent(key, "v1", new EmbeddedMetadata.Builder().lifespan(lifespan).build()));
      CacheEntry cacheEntry = advCache.getCacheEntry(key);
      assertEquals(lifespan, cacheEntry.getMetadata().lifespan());
   }
   ...
   public void testPutAsyncWithLifespan() throws Exception {
      final Integer key = 4;
      long lifespan = 1_000_000;
      Future<String> f = advCache.putAsync(key, "v1", new EmbeddedMetadata.Builder().lifespan(lifespan).build());
      assertNotNull(f);
      assertFalse(f.isCancelled());
      assertNull(f.get());
      assertTrue(f.isDone());
